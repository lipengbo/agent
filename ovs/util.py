...
def start_process(args, **kw):
    ...
    except exceptions.OSError, ex:
        print ex
        return (-1, None, None)

def start_bg_process(args):
    try:
        os.system(' '.join(args)+" &")
        return 0
...
def is_port_exist(match):
    cmd = ("netstat -apn|grep \"%s\"|wc -l") % match
    ret, out, _err = start_process(cmd, shell=True)
    if ret == 0:
        return int(out) == 1
    return False
